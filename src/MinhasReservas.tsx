...
export default function Reservas() {
  ...
  const reservasTable = reservas.map(reservas => (
    <tr key={reservas.id} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
      ...
      <td className="px-6 py-4">
        <p><i>{dataDMA(reservas.dataInicio)}</i></p>
      </td>
      <td className="px-6 py-4">
        <p><i>{dataDMA(reservas.dataFim)}</i></p>
      </td>
      <td className="px-6 py-4">
        <p><i>{(reservas.descricao)}</i></p>
      </td>
      <td className="px-6 py-4">
        <p><i>{(reservas.status)}</i></p>
      </td>
      <td>
      ...
      <td>
        <p className='px-6 py-4'>
          R$: {Number(reservas.valor).toLocaleString("pt-br", { minimumFractionDigits: 2 })}</p>
      </td>
    </tr>
  ))
